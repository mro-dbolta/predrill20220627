...
run_Arps_declines_on_ES =
  function(
    ...
    if (run_if) {
      ...
      source("anonymous_support_functions/_run_Arps_ES_declines.setup_for_oil_fits.R", local = TRUE)
      ...
      source("anonymous_support_functions/_run_Arps_ES_declines.fit_oil_on_individual_api.R", local = TRUE)
      ...
      row_count_test = 
        procount_prod_data_1 %>% 
        ### only keep records with volume
        filter(!is.na(VOLUME)) %>%
        filter(VOLUME > 0.0)
      if (nrow(row_count_test) != nrow(procount_prod_data_1)) stop("Found some prod data rows with missing or 0 volume")
      ...
      all_wells_arps_grid_results_oil %>% 
        fwrite(paste0(output_folder,
                      "ES_wells_arps_grid_results_oil.csv"))
      
      return("Arps run of ES autodeclines successful.")
